...
-- TODO handle `returned ErrorBusy while attempting to perform step: database is locked`
-- Maybe just a problem with SQLite (https://stackoverflow.com/questions/9055214/what-are-the-rules-about-concurrently-accessing-a-persistent-database)
-- TODO compress status into summary
    ...
    \case
      ...
      UpdateEndpoint k e -> embed . runSqlite sqliteDbName $ do
        ...
        where
          ...
          getNewPings before new = nub $ new \\ (pingFromDb . removeIdentity <$> before)  -- TODO this does not scale
